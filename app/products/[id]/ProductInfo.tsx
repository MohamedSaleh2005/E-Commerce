...
import { FaCartArrowDown, FaRegHeart, FaStar, FaStarHalfStroke } from 'react-icons/fa6'
...
export default function ProductInfo({ product }: Props) {
    ...
    return (
        ...
        <div className='w-[50%]'>
            ...
            <button className={`btn my-5 rounded-sm border bg-(--main-color) border-(--main-color) transition-all duration-300  ${IsInCart ? "bg-transparent text-(--main-color) pointer-events-none" : "text-white"} `} onClick={HandleToCart}>{IsInCart ? "Item In Cart" : "Add to Cart "}<TiShoppingCart className='text-[16px]' /></button>
