...
export default function Product({ item }: Props) {
  ...
  return (
    <div className={`relative mb-5 w-45 h-75 bg-(--white-color) pb-1.25 px-3.75 border border-(--border-color) hover:border-(--main-color) rounded-[5px] overflow-hidden hover:shadow-lg transition-all duration-300 Card ${IsInCart ? "In-Cart" : ""}`}>
      <Link href={`/products/${item.id}`}>

      <span className={`flex text-[14px] items-center gap-1 absolute right-[37%] mt-1 font-sans text-(--main-color) transition-all duration-300 ${IsInCart ? "translate-y-0 opacity-100" : "-translate-y-5 opacity-0"} `}><FaCheck className='text-green-500'/>In Cart</span>
      ...
      <div className='absolute top-[40%] text-sm translate-y-[-50%] flex flex-col -right-12.5 gap-3.5 RemoveIcon transition-all duration-400'>
        <span className={`w-7 h-7 hover:text-(--main-color) bg-(--bg-color) flex items-center rounded-full cursor-pointer justify-center transition-all duration-300 ${IsInCart ? "bg-(--main-color) text-white hover:text-white" : ""} `} onClick={() =>   addToCart({ id: item.id, name: item.title, price: item.price})}><FaCartArrowDown /></span>
